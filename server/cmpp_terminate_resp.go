...
var cmppTerminateResp TrafficHandler = func(cmd, seq uint32, buff []byte, c gnet.Conn, s *Server) (next bool, action gnet.Action) {
	if uint32(cmpp.CMPP_TERMINATE_RESP) != cmd {
	...
	err = sc.Pool().Submit(func() {
		handleCmppTerminateResp(s, sc, term)
	})
	if err != nil {
...
func handleCmppTerminateResp(s *Server, sc *session, term *cmpp.TerminateRsp) {

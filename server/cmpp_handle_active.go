...
var cmppActive TrafficHandler = func(cmd, seq uint32, buff []byte, c gnet.Conn, s *Server) (next bool, action gnet.Action) {
	...
	test := &cmpp.ActiveTest{}
	err := test.Decode(seq, buff)
	if err != nil {
	...
	err = sc.Pool().Submit(func() {
		handleCmppActive(s, sc, test)
	})
	if err != nil {
		log.Error(fmt.Sprintf("[%s] OnTraffic %s", sc.ServerName(), RC),
			FlatMapLog(sc.LogSession(32), []log.Field{OpDropMessage.Field(), ErrorField(err), Packet2HexLogStr(test.Encode())})...)
		return false, gnet.Close
	}
...
func handleCmppActive(s *Server, sc *session, test *cmpp.ActiveTest) {
	...
	// 打印登录报文
	log.Debug(msg, FlatMapLog(sc.LogSession(), test.Log())...)
	// n. 发送响应
	resp := test.ToResponse(0)
	pack := resp.Encode()
	// 异步非阻塞

...
func (s *session) completeLogin(ac *codec.AuthConf) {
	// 设置会话信息及会话级别资源，此代码非常重要！！！
	s.Lock()
	defer s.Unlock()
	s.stat = StatLogin
	s.ver = ac.Version
	s.clientId = ac.ClientId
	s.lastUseTime = time.Now()
	s.closeResource()
	s.window = make(chan struct{}, ac.MtWindowSize)
	s.pool = createSessionSidePool(ac.MtWindowSize * 2)
	s.setupLimiter(ac)
}

func (s *session) setupLimiter(ac *codec.AuthConf) {
	// 默认1W微妙即10毫秒生成一个token，也即tps最大200
	ev := 10 * time.Millisecond
	if ac != nil && ac.Throughput != 0 {
		// 1s = 1000*1000 microsecond = 1000000 microsecond, Throughput 单位时TPS
		ev = time.Duration(1000000/ac.Throughput) * time.Microsecond
	}
	limit := rate.Every(ev)
...
func (s *session) LogCounter() []log.Field {
	...
	ac := msc.FindAuthConf(s.serverName, s.clientId)
	if ac == nil {
		ac = &codec.AuthConf{}
	}
	...
	return []log.Field{
		log.Int(LogKeyClientConnsCap, ac.MaxConns),

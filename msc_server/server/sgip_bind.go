...
var sgipBind TrafficHandler = func(cmd, seq uint32, buff []byte, c gnet.Conn, s *Server) (next bool, action gnet.Action) {
	if uint32(sgip.SGIP_BIND) != cmd {
...
func handleSgipBind(s *Server, sc *session, login *sgip.Bind) {
	var msg = fmt.Sprintf("[%s] OnTraffic %s", s.name, RC)
	// 打印登录报文
	log.Info(msg, FlatMapLog(sc.LogSession(16), login.Log())...)

	// 获取客户端信息
	ac := msc.FindAuthConf(s.name, login.LoginName)
	code := login.Check(ac)

	// 检查当前已登录会话数是否已达上限
	if code == sgip.Status(0) {
		// 注意这里仅按照单节点计算某个client的session数，实际上应该计算集群中的某个client的session数。
		// TODO 要支持集群，连接会话的计数，应该采用数据库或者redis等存储。
		activeSession := s.CountSessionByClientId(ac.ClientId)
		if activeSession >= ac.MaxConns {
			code = sgip.Status(3)
		}
	}
	...
	if err != nil {
		log.Error(msg, FlatMapLog(sc.LogSession(), []log.Field{sgip.SGIP_BIND.OpLog(), SErrField(err.Error())})...)
	}
}

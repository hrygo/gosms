...
var smgpLogin TrafficHandler = func(cmd, seq uint32, buff []byte, c gnet.Conn, s *Server) (next bool, action gnet.Action) {
	if uint32(smgp.SMGP_LOGIN) != cmd {
...
func handleSmgpLogin(s *Server, sc *session, login *smgp.Login) {
	var msg = fmt.Sprintf("[%s] OnTraffic %s", s.name, RC)
	// 打印登录报文
	log.Info(msg, FlatMapLog(sc.LogSession(16), login.Log())...)

	// 获取客户端信息
	cli := msc.FindAuthConf(s.name, login.ClientID())
	...
	if err != nil {
		log.Error(msg, FlatMapLog(sc.LogSession(), []log.Field{smgp.SMGP_LOGIN.OpLog(), SErrField(err.Error())})...)
	}
}

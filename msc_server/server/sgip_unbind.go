...
var sgipUnbind TrafficHandler = func(cmd, seq uint32, buff []byte, c gnet.Conn, s *Server) (next bool, action gnet.Action) {
	if uint32(sgip.SGIP_UNBIND) != cmd {
...
func handleSgipUnbind(s *Server, sc *session, term *sgip.Unbind) {
	...
	if err != nil {
		log.Error(msg, FlatMapLog(sc.LogSession(), []log.Field{sgip.SGIP_UNBIND.OpLog(), SErrField(err.Error())})...)
	}
}

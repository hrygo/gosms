...
func TestNewSubmit(t *testing.T) {
	...
	mts := NewSubmit(cli, phones, content, uint32(codec.B32Seq.NextVal()))
	for _, mt := range mts {
		t.Logf(">>> %v", mt)
	...
	mts = NewSubmit(cli, phones, content2, uint32(codec.B32Seq.NextVal()))
	for _, mt := range mts {
		t.Logf(">>> %v", mt)
		t.Logf("<<< %v", mt.ToResponse(0))
	}
	...
	content3 := "强大的祖国"
	mts = NewSubmit(cli, phones, content3, uint32(codec.B32Seq.NextVal()))
	for _, mt := range mts {
		t.Logf(">>> %v", mt)
		bts := mt.Encode()
	...
	mts = NewSubmit(cli, phones, content4, uint32(codec.B32Seq.NextVal()))
	for _, mt := range mts {
		t.Logf(">>> %v", mt)
		t.Logf("<<< %v", mt.ToResponse(0))
	}
...
func TestSubmit_Encode(t *testing.T) {
	...
	mts := NewSubmit(cli, phones, Poem, uint32(codec.B32Seq.NextVal()), MtAtTime(time.Now().Add(5*time.Minute)))
	...
	for _, mt := range mts {
		...
		decMt := &Submit{Version: Version(cli.Version)}

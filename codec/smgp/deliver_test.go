...
func TestDeliver_Decode(t *testing.T) {
	dlv := smgp.NewDeliver(cli, "123", "95535", "TD:123456", uint32(codec.B32Seq.NextVal()))
	t.Logf("dlv: %s", dlv)
	testDeliver(t, dlv)
}

func TestDeliver_ReportDecode(t *testing.T) {
	mts := smgp.NewSubmit(cli, []string{"17011113333"}, "hello world，世界", uint32(codec.B32Seq.NextVal()), smgp.MtOptions{})
	mt := mts[0]
	msp := mt.ToResponse(0).(*smgp.SubmitRsp)
	tm := mt.(*smgp.Submit)
	rpt := smgp.NewDeliveryReport(cli, tm, uint32(codec.B32Seq.NextVal()), msp.MsgId())
	t.Logf("dlv: %s", rpt)
	testDeliver(t, rpt)
}

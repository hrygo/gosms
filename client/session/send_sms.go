...
type Result struct {
	...
	RequestId    uint32    `json:"requestId"`    // 消息发送的标识
...
// RequestIdResultCacheMap 临时存储短信发送的返回结果数据，Key为requestId,value为*Result，后续采用数据库存储
var RequestIdResultCacheMap sync.Map
